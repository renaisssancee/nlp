...
CineMatch — интеллектуальная система рекомендации фильмов, построенная на архитектуре Retrieval-Augmented Generation (RAG). Пользователь описывает на естественном языке (русском или английском), какой фильм хочет посмотреть, а система находит подходящие фильмы из базы TMDB 5000 и объясняет, почему они подходят.
...
14. [Архитектурные решения и компромиссы](#14-архитектурные-решения-и-компромиссы)
...
## 14. Архитектурные решения и компромиссы

| Решение | Обоснование | Компромисс |
|---------|-------------|------------|
| **Гибридный поиск** (вектор + фильтры) | Семантическое понимание + точные ограничения (год, длительность) | Сложнее, чем чистый векторный поиск |
| **Cross-Encoder реранкинг** | Значительно точнее, чем bi-encoder similarity | Доп. latency на инференс |
| **Hallucination Guard** | Предотвращает "натягивание" нерелевантных фильмов | Может отсечь пограничные, но полезные результаты |
| **OpenRouter + free models** | Бесплатно, множество моделей, OpenAI-совместимый API | Зависимость от доступности сервиса, rate limits |
| **Цепочка fallback-моделей** | Высокая доступность при отказе одной модели | Качество может варьироваться между моделями |
| **Локальная ChromaDB** | Нет внешних зависимостей, полная приватность данных | Нет масштабирования, привязка к одной машине |
| **Streamlit** | Быстрое прототипирование, встроенное управление состоянием | Ограниченная кастомизация UI |
| **SQLite для логов** | Лёгкий, не требует отдельного сервера | Не подходит для распределённого логирования |
| **`semantic_query` на английском** | Эмбеддинги `all-MiniLM-L6-v2` обучены преимущественно на английском | Требует перевода запроса через LLM |
| **Pipe-delimited жанры** | Надёжная фильтрация через `$contains` в ChromaDB | Нестандартный формат, требует конвертации |
